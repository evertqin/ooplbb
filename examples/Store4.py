...
"""
Extract Method (110)
Create Customer.get_total_charge()
Create Customer.get_total_frequent_renter_points()

Replace Temp with Query (120)
Changed totalAmount          to get_total_charge()
...
class Rental (object) :
    ...
    def get_frequent_renter_points (self) : # const
        if (self.get_movie().get_price_code() == Movie.NEW_RELEASE) and \
           (self.get_days_rented()            >  1) :                     # why not self.movie, self.days_rented?
            return 2
        return 1
